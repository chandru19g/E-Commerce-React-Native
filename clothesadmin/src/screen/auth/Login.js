  ...
  const getUser = async () => {
    try {
      ...
      if (value !== null) {
        ...
        if (value.user.role === 0) Alert.alert('Wrong', 'Admin ID');
        else if (value.user.role === 1) navigation.replace('Admin');
      } else {
        setUser(null);
      }
    } catch (e) {
    ...
    login(input)
      .then(result => {
        ...
        if (result.error) {
          ...
          if (result.user.role === 1) navigation.navigate('Admin');
          else Alert.alert('Wrong', 'Admin ID');
